...
for file in allFilesL:
    ...
    for i in dataFrameTempToTalLP.index:
        eachData=dataFrameTempToTalLP.loc[i]
        infor_list=eachData['행정구역'].split(" ")
        data={'지역':infor_list[1],'행정동코드':infor_list[2][1:6],'2020년08월_남자 인구수':\
            eachData['2020년08월_남자 인구수'], '2020년08월_여자 인구수':eachData['2020년08월_여자 인구수'] }
        dataFrameTotalLp.append(pd.DataFrame(data,\
            columns=['지역','행정동코드','2020년08월_남자 인구수','2020년08월_여자 인구수'], index=[i]))

dataFrameTotalLpCSV=pd.concat(dataFrameTotalLp,axis=0,ignore_index=True)
output="C:\\Users\\admin\\Desktop\\WaRaConv\\ProcessedDataSet\\서울시 8월 구별 주민등록인구(성별).csv"
